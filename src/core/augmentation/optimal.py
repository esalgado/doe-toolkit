...
def _augment_split_plot(
    ...
    # original_design is always in natural units (invariant since Phase 2),
    # and new_runs is built from actual-value candidates, so no re-encoding needed.
...
def augment_for_model_extension(
    ...
    # original_design is always in natural units (invariant since Phase 2);
    # encode to coded space for candidate exclusion comparison.
    from src.core.coding import encode_design
    ...
    # original_design is always in natural units (invariant since Phase 2);
    # decode candidates from coded space to actual values to match.
